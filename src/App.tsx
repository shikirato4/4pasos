...
const stages = [
  ...
  { label:'PUNTO DE VENTA', title:'En tu\ntienda.', text:'La lata llega a la tienda de barrio. Allí se exhibe, se vende y pasa a las manos del consumidor.', note:'LA ESQUINA · TIENDA DE BARRIO',a:.88,b:.965 },
]
...
export function App() {
  ...
  return <main className="story" aria-label="El proceso productivo del aluminio"><div className="stage-viewport">
    ...
    <motion.div className="ending-shade" style={{opacity:dark}}/><motion.section className="outro" style={{opacity:outro}}><div className="eyebrow">CUATRO ETAPAS. UNA TRANSFORMACIÓN.</div><h2>DE LA MATERIA PRIMA<br/><span>AL PRODUCTO FINAL.</span></h2><p>BAUXITA <span>→</span> ALUMINIO <span>→</span> LATA <span>→</span> TIENDA</p><button onClick={()=>go(0)} disabled={percent<97}>VOLVER AL ORIGEN <span>↗</span></button></motion.section>
    <a className="asset-credit" href="./models/CREDITS.txt" target="_blank" rel="noreferrer">Lata 3D: William Prosser · CC BY 4.0 ↗</a><aside className="side-note">MATERIA EN TRANSFORMACIÓN <span> / </span> Al — 26.98</aside><footer><div className="footer-top"><span>EL RECORRIDO</span><span>{String(percent).padStart(3,'0')}<i> %</i></span></div><nav aria-label="Etapas del proceso" className="stage-nav"><div className="track"/><motion.div className="track fill" style={{scaleX:raw}}/>{['Bauxita','Aluminio','Lata','Tienda'].map((s,i)=><button key={s} onClick={()=>go(stops[i])} aria-current={stage===i?'step':undefined}><span className="nav-dot"/><span className="nav-number">0{i+1}</span><span className="nav-label">{s}</span></button>)}</nav></footer>
  </div></main>
}
